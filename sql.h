...
public:
	...
	bool save() {
		if (roomNumber == 0) {
			qout << QFgColor(0xff, 0, 0) << "roomNumber not exists in roomstatu" << QResetColor();
			return false;
		}
		else {
...
public:
	...
	bool save() {
		if (roomNumber == 0 || price == 0 || vip == 0) {
			qout << QFgColor(0xff, 0, 0) << "roomNumber/price/vip has not exists in room" << QResetColor();
			return false;
		}
		else {
...
public:
	...
	bool save() {
		if (recordTime == "") {
			qout << QFgColor(0xff, 0, 0) << "recordTime not exists in roomer" << QResetColor();
			return false;
		}
		else {
...
public:
	...
	bool save() {
		if (roomNumber == 0) {
			qout << QFgColor(0xff, 0, 0) << "roomNumber not exists in event" << QResetColor();
			return false;
		}
		else {
